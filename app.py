...
Changes vs v2:
  • Live prices via multi-source waterfall (Stooq → Yahoo JSON → Boursorama).
    No hardcoded fallback values — if all sources fail for a ticker, the app
    displays a clear error for that instrument.
  • MASTER table rebuilt with EUR return columns (P&L, YTD, 1M, 1D) and
    portfolio-weight column. Includes a TOTAL row.
  • Annualization fixed: only applied when holding >= 180 days; short-hold
    rows show raw period return (annualization is meaningless for 6-week
    positions).
  • New table: per-instrument annual P&L in EUR with a Total column and
    Total row.
"""
...
TRADING_DAYS = 252
MIN_ANNUALIZE_DAYS = 180  # Don't annualize positions held less than this
...
def fetch_price_history(tickers: tuple[str, ...], start: date, end: date) -> pd.DataFrame:
    return price_feed.fetch_history(list(tickers), start, end)
...
def build_positions(transactions: pd.DataFrame, price_index: pd.DatetimeIndex) -> pd.DataFrame:
    """Daily cumulative quantity held, by ticker."""
...
def compute_portfolio_series(
    ...
    """Compute daily portfolio MV, net invested (from user-entered tx prices),
    daily cashflows, and time-weighted daily returns.

    The LAST day's portfolio value uses the latest live price (from `current_prices`)
    for each ticker, which overrides the ffill'd historical price. This ensures
    KPIs reflect the real current market value, not a possibly-stale last close."""
    common = [t for t in positions.columns if t in prices.columns]
    pos = positions[common]
    px = prices[common].copy()

    # Override the last row with the live current prices where available
    last_idx = px.index[-1]
    for t in common:
    ...
    # Cashflows using USER-ENTERED prices
    tx = transactions.copy()
    tx["Date"] = pd.to_datetime(tx["Date"])
...
def instrument_value_series(
    ...
    """Per-instrument daily market value (pos × price). Last row uses live prices."""
    common = [t for t in positions.columns if t in prices.columns]
    pos = positions[common]
    px = prices[common].copy()
    last_idx = px.index[-1]
...
def instrument_cashflow_series(
    ...
    """Per-instrument daily cashflow series (cumulative invested per instrument)."""
    tx = transactions.copy()
    tx["Date"] = pd.to_datetime(tx["Date"])
    tx["Cashflow"] = tx["Quantity"] * tx["Price"]
...
def annualize(total_return: float, days: float) -> float | None:
    """Return None if holding too short to annualize meaningfully."""
...
# =====================================================================
# Data load (from Google Sheets or local cache)
# =====================================================================
...
if not transactions.empty:
    ...
    # Current prices — critical, can partially fail
    ...
    with st.spinner("Fetching price history…"):
        history = fetch_price_history(tuple(tickers_used), start_date, today)
    ...
    for t in tickers_used:
        if t not in history.columns:
            if t in current_price_series.index:
                idx = history.index if not history.empty else pd.bdate_range(start=start_date, end=today)
                history = history.reindex(idx) if not history.empty else pd.DataFrame(index=idx)
...
with tab_master:
    if not compute_ok:
        ...
        # Snap to nearest actual trading date on or before month_ago
        ...
        # Price-source table — show provenance
        st.markdown("---")
        ...
        st.dataframe(
            df_feed, hide_index=True, use_container_width=True,
            column_config={
        ...
        # Best / worst / largest via YTD €
        per_inst = {}
        for t in tickers_used:
            qty = float(positions[t].iloc[-1]) if t in positions.columns else 0
            if qty == 0:
                continue
            p_now = float(inst_values[t].iloc[-1]) / qty if qty != 0 else 0
            try:
            ...
            # Cashflow contribution since YTD start (new money shouldn't count as return)
            ...
            per_inst[t] = {
                "ytd_pnl": ytd_pnl, "ytd_ret": ytd_ret_inst,
        ...
        # ==========================================================
        # NEW: Per-instrument table with EUR return columns + TOTAL row
        # ==========================================================
        st.markdown("---")
        ...
        for t in tickers_used:
            ...
            if qty == 0:
                ...
                if total_sold > 0 and wavg_cost and not pd.isna(wavg_cost):
            ...
            # Period returns in EUR (price movement × qty, minus fresh cashflows in period)
            ...
            # Annualized return — use weighted-avg buy date, skip if too short
            if buys["Quantity"].sum() > 0:
            ...
            rows.append({
                ...
                "Cur Price": cur_px,
                "Market Value": mv,
                "% of Port.": None,  # filled after loop
                "P&L (€)": total_pnl_eur,
                "YTD (€)": ytd_eur,
                "1M (€)": m1_eur,
                "1D (€)": d1_eur,
        ...
        tot_ytd = df_inst["YTD (€)"].sum()
        tot_m1 = df_inst["1M (€)"].sum()
        tot_d1 = df_inst["1D (€)"].sum()
        # Weighted-average cost across open positions (not very meaningful as a scalar — leave blank)
        tot_cost = sum(
            float(r["Market Value"]) - float(r["P&L (€)"])
        ...
        total_row = pd.DataFrame([{
            ...
            "Cur Price": None,
            "Market Value": tot_mv,
            "% of Port.": 100.0 if tot_mv > 0 else 0.0,
            "P&L (€)": tot_pnl,
            "YTD (€)": tot_ytd,
            "1M (€)": tot_m1,
            "1D (€)": tot_d1,
        ...
        st.dataframe(
            ...
            column_config={
                ...
                "Annualized": st.column_config.NumberColumn(
                    format="%.2f%%",
                    help=f"Blank when position held <{MIN_ANNUALIZE_DAYS} days (annualization is misleading for short periods).",
                ),
            },
        )

        # ==========================================================
        # NEW: Annual P&L per instrument, with Total row AND Total col
        # ==========================================================
        st.markdown("---")
        st.subheader("📅 Annual P&L by Instrument (€)")
        st.caption(
            "Each cell = price-based return on the start-of-year position, "
            "plus any new cashflows into that instrument during the year. "
            "Current year shows YTD."
        )
        ...
        for t in tickers_used:
            ...
            for y, col_name in zip(years, cols_year):
                ...
                # Values at start / end of year window
            ...
            row["Instrument"] = INSTRUMENTS[t]["name"]
            annual_rows.append(row)

        df_annual = pd.DataFrame(annual_rows)
        # Reorder: Ticker, Instrument, year cols, Total
        col_order = ["Ticker", "Instrument"] + cols_year + ["Total"]
        df_annual = df_annual[col_order]

        # Total row (sum across instruments per year)
        ...
        st.dataframe(
            ...
            column_config={col: st.column_config.NumberColumn(format="€%.0f") for col in num_cols},
        )
...
with tab_tx:
    ...
    st.caption("Edit the table directly or use the quick-add form below. Changes auto-save.")
